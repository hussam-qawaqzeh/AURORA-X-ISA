module decoder (
    ...
    // For STORE.X (0x22), BRANCH.X (0x40), CSR.WRITE (0x43), and VSTORE (0x61), rs1 is in 23:19
    assign rs1      = (opcode == 8'h22 || opcode == 8'h40 || opcode == 8'h43 || opcode == 8'h61) ? inst[23:19] : inst[18:14];
    // For STORE.X (0x22), BRANCH.X (0x40), and VSTORE (0x61), rs2 is in 18:14
    assign rs2      = (opcode == 8'h22 || opcode == 8'h40 || opcode == 8'h61) ? inst[18:14] : inst[13:9];
    ...
    always @(*) begin
        ...
        case (opcode)
            8'h01: begin // ADD.X / SUB.X
                RegWrite = 1;
                ALUSrc_B = 0;
                ALU_Op   = (funct9 == 9'd1) ? 3'b001 : 3'b000;
            end
            8'h02: begin // AND
                RegWrite = 1; ALUSrc_B = 0; ALU_Op = 3'b010;
            end
            8'h03: begin // OR
                RegWrite = 1; ALUSrc_B = 0; ALU_Op = 3'b011;
            end
            8'h04: begin // XOR
                RegWrite = 1; ALUSrc_B = 0; ALU_Op = 3'b100;
            end
            8'h05: begin // SHL
                RegWrite = 1; ALUSrc_B = 0; ALU_Op = 3'b101;
            end
            8'h06: begin // SHR
                RegWrite = 1; ALUSrc_B = 0; ALU_Op = 3'b110;
            end
            8'h09: begin // ADDI
                RegWrite = 1; ALUSrc_B = 1; ALU_Op = 3'b000; // ADD
            end
            8'h21: begin // LOAD.X
                RegWrite = 1; ALUSrc_B = 1; ALU_Op = 3'b000; // rs1 + imm14
                MemRead  = 1; MemtoReg = 2'b01;
            end
            8'h22: begin // STORE.X 
                MemWrite = 1; ALUSrc_B = 1; ALU_Op = 3'b000;
            end
            8'h40: begin // BRANCH.X
                Branch = 1; ALUSrc_B = 0; ALU_Op = 3'b001; // SUB to test equality
            end
            8'h41: begin // JUMP.X
            ...
            8'h60: begin // VLOAD
                ALUSrc_B = 1; ALU_Op = 3'b000; // rs1 + imm14
                VectorMemRead = 1; VectorRegWrite = 1;
            end
            8'h61: begin // VSTORE
                ALUSrc_B = 1; ALU_Op = 3'b000;
                VectorMemWrite = 1;
            end
            8'h62: begin // VADD
